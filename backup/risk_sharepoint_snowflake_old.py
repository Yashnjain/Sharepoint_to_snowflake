    ...
    for i, x in df_driver.iterrows():
        ...
        try:
            ...
            logging.basicConfig(
                ...
                format='%(asctime)s [%(levelname)s] - %(message)s',
                filename=log_file_location)
            bu_alerts.bulog(process_name='RISK_SHAREPOINT_SNOWFLAKE', database='POWERDB', status='Started',
            ...
            if len(df) < min_rows:
                ...
                if len(to_addr) > 0:
                    ...
                    bu_alerts.send_mail(
                        ...
                        mail_subject='JOB FAILED RISK_SHP->SF - {0}'.format(
                            schemaname+"."+tablename),
            ...
            # Bulog complete job
            j = '[' + json.dumps(variable_dict) + ']'
            bu_alerts.bulog(process_name='RISK_SHAREPOINT_SNOWFLAKE', database='POWERDB', status='Completed',
                            table_name=schemaname + '.' + tablename, row_count=len(df), log=str(j), warehouse="ITPYTHON_WH", process_owner='IMAM')
            if len(to_addr) > 0:
                bu_alerts.send_mail(
                    receiver_email=to_addr,
                    mail_subject='JOB SUCCESS RISK_SHP->SF - {0}'.format(
                        schemaname+"."+tablename),
